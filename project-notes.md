...
トップページのヒーローロゴは、`index.html` 内で「DIRECTORS’ / TIMELINE / ARCHIVE」を3行のHTMLテキストとして組む。Figmaの Inter Bold・文字間隔4%・行間比率を基準に、PCでは元指定の約70%となる62px・行間63pxで表示し、狭い画面では同じ比率を保って縮小する。通常文字とリビールブロックは `#48434c`、各行の頭文字は既存ロゴと同じ金色の濃淡を使う。初回表示では各行を上から順にブロックリビールし、`prefers-reduced-motion` では演出を無効にする。説明キャプションもHTMLテキストとして扱う。

...
トップページのヒーローロゴは、`index.html` 内で「DIRECTORS’ / TIMELINE / ARCHIVE」を3行のHTMLテキストとして組む。Figmaの Inter Bold・文字間隔4%・行間比率を基準に、PCでは68px・行間69pxで表示する。スマホではフッターのロゴと同程度の大きさを目安に36〜40pxへ縮小する。通常文字とリビールブロックは `#48434c`、各行の頭文字は既存ロゴと同じ金色の濃淡を使う。初回表示では各行を上から順にブロックリビールし、`prefers-reduced-motion` では演出を無効にする。トップページのフッターロゴも同じ構成のHTMLテキストとし、アニメーションは適用しない。説明キャプションもHTMLテキストとして扱う。

ヒーロー背景には、複数の縦軸と年号が異なる速度で上方向へ流れるモーショングラフィックを配置する。前景・中景・遠景で年号サイズ、線の濃度、移動速度を変えて奥行きを表現し、ロゴと導線の背後には可読性を保つフェードを置く。装飾用の年号は `home.js` で生成し、`prefers-reduced-motion` ではアニメーションを停止する。
...
グローバルメニューとfooterの主要導線は日本語表記で揃える。
...
グローバルメニューはPCでは横並び、スマホではロゴとハンバーガーボタンを表示し、開いたときに同じ主要導線を縦並びで表示する。スマホメニューでは主要導線のあとに少し間を開けて、`このサイトについて`、`お問合せ`、`@DTAarchive` も表示する。

ヘッダーのHTMLと開閉処理は `navigation.js` で共通管理する。各HTMLには `.header-shell[data-site-header]` の配置場所だけを置き、`notes/` 配下では `data-root="../"` を指定する。メニュー項目を変更するときは、各HTMLへ同じコードを複製せず `navigation.js` の共通テンプレートを更新する。
...
PC表示の年号は28pxを基準とし、中央軸上での視認性を確保する。

固定表示する監督名・同時代作品の見出し領域では、背後に隠れた作品リンクへクリックを透過させない。
...
3. `テーマから比較してみる`
   ...
   - 対比カードでは監督名を背景・枠・ピル装飾のないテキストだけで構成し、日本語名の下に英語名を12px・Secondaryカラーで表示
4. `Editor's Notes`
   - 最新コラムを1件だけプレビュー表示
...
- 各監督は日本語名の直下に英語名を12px・Secondaryカラー・装飾なしで表示する。
- 検索対象は英語名、日本語名、特徴タグ。
...
- 日付ベースの映画史イベントをサイト内データとして管理する。

主な表示:

- ページタイトル: `今日は何の日`
- サブタイトル: `その日に起きた映画史の出来事をたどります。`
- 今日の日付
- 該当イベント一覧
- イベントがない場合の空表示

イベントデータは `data/events.json` から読み込む。日付照合は `MM-DD` 形式。

テスト用に `today.html?date=07-16` のようなクエリで日付を指定できる。
...
今日は何の日用のイベントは `data/events.json` で管理する。

基本形式:

```json
{
  "id": "inception-us-release",
  "date": "07-16",
  "year": 2010,
  "type": "movie_release",
  "title": "『インセプション』全米公開",
  ...
  "description": "クリストファー・ノーラン監督の代表作のひとつ。キャリア上の重要作。",
  "country": "US",
  "source": "",
  "links": []
}
...
`links` は関連するタイムラインやEditor's Notesへ誘導するための余地として使う。

### Editor's Notesデータ

Editor's Notes のトップページプレビューと一覧表示は、`editor-notes.js` の `editorNotes` 配列で管理する。

`editor-notes.js` には一覧表示に必要なメタ情報だけを置く。本文は各 `notes/*.html` に集約し、JSとHTMLで本文を二重管理しない。
...
- `data/events.json` に追加する。
- 監督名を受け取ったら、まず既存の `home.js` / `main.js` と照合し、監督IDや表記を確認する。
- その監督にまつわるイベントを調べ、公式情報や信頼できる情報源で日付を確認する。
- 追加できる主なデータは、監督の誕生日、監督の命日、代表作・重要作の公開日、受賞日、映画史的な出来事。
- 既存データと重複しない `id` を付ける。
- `date` は必ず `MM-DD` 形式にする。
- `type` は既存の想定typeから選ぶ。
- X投稿文として使われることを考え、`description` は短く明確にする。
- 事実確認が必要な日付には、できるだけ `source` を入れる。
- 関連監督のタイムラインやEditor's Notesがある場合は `links` に入れる。
- 追加後は JSON 構文チェックを行う。
...
`notes/index.html` は この監督のこの一本 一覧ページ。
...
- ヘッダーロゴは `assets/header_logo.svg` を使い、ブラウザ左側に配置する。
- PCのグローバルメニュー項目は `1100px` のコンテンツ幅内で左寄せにする。
- スマホではハンバーガーボタンから同じ項目を縦並びで開く。
- `ホーム`
- `映画監督から選ぶ`
- `年代から探す`
- `今日は何の日`
...
JS更新後は、必要に応じて `home.js?v=...`、`main.js?v=...`、`years.js?v=...`、`editor-notes.js?v=...`、`navigation.js?v=...` を更新する。

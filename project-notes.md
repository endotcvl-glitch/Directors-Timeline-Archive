...
footerには `assets/header_logo.svg` のロゴ、上記導線に加えて、`このサイトについて`、`お問合せ`、Xアカウントへのリンクを置く。基本的に公開ページには共通footerを配置する。
...
Today in Film History / 今日は何の日ページ。
...
Today in Film History 用のイベントは `data/events.json` で管理する。
